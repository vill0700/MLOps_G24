...
3. **Model Training**: We use the transformed data to train a machine learning model. It is simply a logistic regression that takes the 1024 dim embeddings and outputs logits for the 22 classes. The model can be quantized to int8 weights and pruned for the 20% smallest weights. Our full model therefore is a frozen SentenceTransformer with a linear classification head.
